...
namespace Swashbuckle.AspNetCore.ApiTesting
{
    public static class HttpRequestMessageFactory
    {
            ...
            object requestBody = null)
        {
            ...
            var parameterSpecs = ExpandParameterSpecs(openApiDocument, pathSpec, operationSpec);
            ...
            Dictionary<string, object> requestParameters = null)
        {
            ...
            var queryString = string.Join('&', parameterSpecs
                .Where(p => p.In == ParameterLocation.Query && requestParameters.ContainsKey(p.Name))
                .Select(p => $"{p.Name}={requestParameters[p.Name].ToString()}"));
